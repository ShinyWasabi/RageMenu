...
PROC FEATURES_VEHICLE_TELEPORT(VEHICLE_INDEX viVehicle)
    IF NOT IS_ENTITY_A_VEHICLE(viVehicle)
        EXIT
    ENDIF

    INT iSeatIndex
    IF IS_VEHICLE_SEAT_FREE(viVehicle, -1, TRUE)
        iSeatIndex = -1
    ELIF IS_VEHICLE_SEAT_FREE(viVehicle, -2, TRUE)
        iSeatIndex = -2
    ELSE
        EXIT
    ENDIF

    VECTOR vLocation = GET_ENTITY_COORDS(viVehicle, TRUE)
    UTIL_TELEPORT_LOAD_GROUND_AT_3DCOORD(vLocation)

    SET_ENTITY_COORDS(PLAYER_PED_ID(), vLocation, TRUE, FALSE, FALSE, TRUE)
    WAIT(0)
    SET_PED_INTO_VEHICLE(PLAYER_PED_ID(), viVehicle, iSeatIndex)
ENDPROC
